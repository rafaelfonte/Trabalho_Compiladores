...
comp_tree_t* arvoreCriaNodo(int qtdeFilhos,int tipo) {
   ...
   novoNodo->label_true = NULL;
   novoNodo->label_false = NULL;
   return novoNodo;
}
...
comp_tree_t* arvorePegaUltimoFilho(comp_tree_t* Nodo){
	comp_tree_t* ptaux = Nodo->filhos[0];
	comp_tree_t* aux = NULL;
	int i = 1;

	while(ptaux != NULL && i < Nodo->qtdeFilhos){
		aux = ptaux;
		ptaux = Nodo->filhos[i];
		i++;
	}
